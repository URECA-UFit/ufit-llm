...
from ufit.dto.recommend import RecommendResponse, PlanDTO
...
def search_plans_node(state: State, k: int = 2):
    ...
    user_msgs = [msg.content for msg in state["messages"] 
...
def recommend_response_node(state: State):
    ...
    plans_text = "\n\n".join(plan_blocks)
    prompt = get_prompt(history_text, state["user_info"], plans_text, state["raw_message"])
...
def search_branch(state: State):
    ...
    if(state["is_recommend"]==True and state["threshold_met"]==False): return "condition1"
    elif(state["is_recommend"]==True and state["threshold_met"]==True): return "condition2"
    elif(state["is_recommend"]==False and state["threshold_met"]==False): return "condition3"
    elif(state["is_recommend"]==False and state["threshold_met"]==True): return "condition4"
...
def make_recommend(user_id: int,base_prompt: str, chat_room_id: int, postgre_db: Session, mongo_db: Database):
