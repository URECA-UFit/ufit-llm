...
def get_other_carrier_prompt(input: str):
    ...
    prompt = ChatPromptTemplate.from_messages([
        SystemMessagePromptTemplate.from_template(
            """당신은 사용자의 질문이 LG U+ 외의 **타 통신사(SKT, KT, 알뜰폰)**와 관련된지 판단하는 분류 어시스턴트입니다.

다음과 같은 경우에만 "타 통신사 관련 질문"으로 판단하세요:
✅ **명확하게 SKT, KT, 알뜰폰이 언급된 경우**
✅ **통신사 이름을 포함한 요금제 비교 요청이 있는 경우**

다음과 같은 질문은 "LG U+ 전용 질문"으로 판단해야 합니다:
🚫 통신사 언급 없이 "제일 싼 요금제", "무제한 요금제 추천"처럼 일반적인 표현만 있는 경우
🚫 "유플러스", "U+"가 언급된 경우
...
3. 비회원이 아닌 사용자인 경우, 해당 사용자의 현재 요금제 이름을 가끔은 사용할 수 있습니다.
4. 만약에 사용자 질문과 후보 요금제가 사용자와 부합하지 않는다면 돌려 말해서 추천해주세요!
5. 반드시 프론트에서 요금제 카드를 렌더링할 수 있도록 아래 형식만 단독으로 출력합니다 (중괄호, 대괄호 등 제거):
   [[RECOMMENDATION_LIST]]
6. 위 형식 이후에는 절대로 요금제 이름이나 가격 등의 정보 추가 설명 금지
7. 마지막 문장은 '보다 정확한 추천을 원하신다면 사용 목적이나 선호 조건을 더 알려주세요!'로 고정합니다.
